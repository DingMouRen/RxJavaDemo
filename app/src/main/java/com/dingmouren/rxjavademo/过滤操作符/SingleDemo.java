 ...
 * single()操作符：在源Observable只发射一个数据项的时候，single()操作符会将这个数据提交给订阅者，大于1个就抛Sequence contains too many elements的异常
 * single(Func1)操作符是对源Observable发射的数据项进行判断，如果符合条件的数据数量大于1就会抛异常。
